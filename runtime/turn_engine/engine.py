...
# coworker/engine.py @ 54b4bfd82d75704a4079ecea4f8f622aa152dbde.
...
class TurnEngine:
    ...
    def switch_model(self, model: str) -> Optional[str]:
        ...
        if not had_history:
            return None
        text = f"Model switched to {model}"
        try:
    ...
    async def _loop(self) -> AsyncIterator[Event]:
        ...
        while True:
            ...
            try:
                ...
                public_error = friendly or "model request failed"
                payload = {
                    "error": public_error,
                    "error_type": "provider_error",
                }
                self._append_notice("error", public_error)
    ...
    async def _handle_tool_calls(
        ...
        for tool_call in tool_calls:
            ...
            if not allowed:
                continue
            # Interactive controls still pass through the manifest-bound registry
            # and PermissionEngine. Their out-of-band user decision is the final
            # consent, so only their execution path is specialized here.
