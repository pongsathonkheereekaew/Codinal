...
- 1.2 แยก `SessionManager` (`server/manager.py` ~3,300 จริง, ไม่ใช่ `sessions.py`) → EXTRACT boundary slices + REWRITE glue (per `docs/plan/openworker-boundary-map.md`). ทำทีละ vertical slice พร้อม test/commit แยก โดยเริ่ม `sessions`; checkout source ที่ pin ใน `/tmp` แล้ว copy เฉพาะ module ที่ใช้พร้อม provenance header (ไม่ commit source tree ทั้งก้อน). **Progress:** `sessions`, `events`, non-secret `settings` และ core composition root อยู่ใน `runtime/` แล้ว; engine build ถูกบังคับผ่าน `PermissionEngine` + injected/default-deny `Approver`; `automations` defer post-MVP ตาม D9; scoped `connectors-gateway` และ control-plane integration ยัง pending.
- 1.3 ✅ Control plane auth (F1-reopen): Tauri v2 host mint token แบบ 256-bit ต่อ process, ส่งให้ Python sidecar ผ่าน child environment (ไม่อยู่ใน argv), inject เข้า WebView memory และบังคับ auth middleware ครอบ HTTP ทุก route + 2 WS. HTTP ใช้ `Authorization: Bearer`; browser WS ใช้ `Sec-WebSocket-Protocol` (`codinal.v1`, `codinal.auth.<token>`); query-string token ถูกปฏิเสธ, CORS/WS Origin จำกัด Tauri/dev origins, docs/OpenAPI ปิด. Python bind เฉพาะ `127.0.0.1`; CSP เปิด. Gate: 32 control-plane tests + Rust security tests.
- 1.4 ✅ Keychain secret adapter (แก้ P1 plaintext): Rust host ใช้ native macOS Security.framework; ไม่มี secret ใน argv/env/disk. Startup bootstrap ส่งผ่าน one-shot stdin pipe แล้ว Python เก็บใน memory เท่านั้น; hot update ทำ Keychain + authenticated runtime sync แบบ rollback เมื่อ sync ล้มเหลว. Mutation ต้องใช้ second sync token ที่ไม่ inject เข้า WebView; API/status ไม่คืนหรือ echo secret.
- 1.5 ✅ OAuth callback lifecycle (แก้ P0): native
  `codinal://oauth/callback` deep link → strict Rust parser → dual-token loopback
...
- 2.1 vendor TurnEngine + providers (Anthropic/OpenAI/Gemini/Ollama) พร้อม
  ...
  secure router + loopback-only Ollama เสร็จแล้ว. TurnEngine ถูก vendor/adapt
