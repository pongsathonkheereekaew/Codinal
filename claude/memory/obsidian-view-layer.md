...
Obsidian was added as a read/graph VIEW over the curated memory folder `~/.claude/projects/-/memory/` (registered as an Obsidian vault, `.obsidian/` config in place). claude-mem stays the episodic capture/recall engine — Obsidian does NOT capture; it only visualizes/edits the durable facts that are already there.
