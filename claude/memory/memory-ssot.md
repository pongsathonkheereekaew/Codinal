...
**How to apply:** never enable a competing memory/second-brain system (harness-mem, openwolf cerebrum, ECC continuous-learning observe, etc.). If a tool bundles its own memory, disable that part. Per-repo `docs/wiki/` engineering pages are fine — they are git docs, not a memory capture engine (see [[project-wiki]]). See [[workflow-stack]].
