...
**persona ⊇ skill:** same YAML-frontmatter + markdown body shape as `SKILL.md`, with extra structured fields. Invalid manifests should fail loudly, not silently.
...
| `default_permission_mode` | `discuss` \| `plan` \| `interactive` \| `custom` \| `auto` | See `agent-guardrails` |
